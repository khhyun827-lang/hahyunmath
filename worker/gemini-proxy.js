...
**① 그림이 «좌표평면 위의 함수 그래프»이면 무조건 (B)입니다.** 다른 갈래를 따지지 마세요.
  - 함수 그래프란: x축·y축이 있는 좌표평면 위에 직선·포물선·곡선(다항함수, 유리·무리함수, 지수·로그, 삼각함수)이 그려진 그림입니다. 교점·꼭짓점·절편이 표시되어 있어도 그렇습니다.
  ...
  - ⚠ 좌표평면 위라도 함수 그래프가 아닌 것(삼각형·원·선분의 위치 관계, 색칠한 영역의 넓이, 점만 찍힌 그림)은 여기 들지 않습니다 — ②로 가세요.
...
(B) 남지 않는다 → "mode"를 "figure"로 하세요. 아래 중 하나라도 해당하면 반드시 (B)입니다.
  ...
  - 함수 그래프의 figureSpec에는 **식을 세울 수 있는 수**를 적으세요 — x절편·꼭짓점·교점의 x좌표·특정 x에서의 값처럼 «검산할 수 있는 조건»이어야 기계가 그립니다. 「대략 오른쪽으로 옮긴다」 같은 말로는 못 그립니다.
