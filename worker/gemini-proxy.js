...
async function handleGeminiTwin(request, env, corsHeaders) {
  ...
  const reverseEngineerRule = `문제를 변형할 때 임의의 난수를 대입하지 마세요. 먼저 최종 정답이 "깔끔한 정수" 또는 "간단한 기약분수"로 떨어지도록 목표 정답을 설정한 뒤, 그 정답이 도출되도록 원본 문제의 조건(방정식의 계수, 평행이동 좌표, 도형의 길이 등)을 역산하여 재구성하세요.`;
...
{"problem": "새 문제 내용", "answer": "새 정답", "solution": "단계별 풀이 과정을 1. 2. 3. 처럼 번호를 매겨 서술 (검토자가 정답을 검증할 수 있도록, 마크다운 기호 없이 일반 텍스트로)"}`;
  ...
  const taskRule = hasImage
    ? `다음은 고등학교 수학 문제와 정답이고, 문제에 첨부된 그림(그래프/도형)도 같이 드립니다. 문제의 풀이 구조, 유형, 난이도는 그대로 유지하되 숫자(계수, 상수, 조건 값 등)만 바꾸어 "쌍둥이 문제" 1개를 새로 만들어주세요. 그림은 새로 그릴 수 없으니, 그림 자체는 그대로 두고 그림과 무관하게 바뀔 수 있는 숫자만 변형하거나, 그림에 의존하지 않는 형태로 문제를 조정해주세요.`
...
${answer}`;
  ...
  return new Response(JSON.stringify({ content: parsed.problem, answer: parsed.answer, solution: parsed.solution || '' }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}
