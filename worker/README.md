...
- [x] **배포본에서 AI 생성 · 이미지 업로드 정상 동작 확인 (2026-08-07).**
      막히는 쪽과 통하는 쪽이 둘 다 확인됐다
...
⚠ **한도 숫자는 최대 1분 늦는다.** KV 읽기가 최대 60초 캐시된다 —
  검토를 한 건 쓴 «직후»에 물으면 0 으로 보인다(2026-09-07 실제로 그랬다).
  「셈이 안 된다」고 판단하기 전에 1분 뒤에 다시 볼 것. 생성 통(`ai`)도 마찬가지다.

또는 한 번에 확인한다 — `node tools/review-check.mjs [--try]`
  인자 없이는 공짜다(`/quota` 는 세지 않는다). `--try` 만 한 건을 쓴다.
