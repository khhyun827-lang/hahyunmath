...
function convertHwpEq(script){
  ...
  /* 🔴 **한글 전용 영역(PUA) 글자가 섞여 온다** (2026-09-06 · 사용자가 뜻을 확인해 주었다).
     ...
     사용자가 「그렇게해줘」로 확인해 주어서 넣는다.
     ⚠ **확인한 표본은 하나뿐이다**(창고 수식 7638개 중 1개). 다른 PUA 글자가 나오면
       여기에 «덧붙이기 전에» 그때도 물을 것 — 뜻을 짐작해 넣으면 조용히 틀린 문제가 된다.
     ⚠ 이 표는 `hwpxRepairEqText` 도 같이 쓴다 — 한쪽에만 넣으면 새로 올린 교재와
       고쳐 둔 창고가 서로 다른 글이 된다. */
  const HWP_PUA = { '\uE04D': '\\vert ' };
  for(const 글자 in HWP_PUA) s = s.split(글자).join(HWP_PUA[글자]);
...
function hwpVerdictBlockers(v, 옵션){
  /* 🔴 **«코드를 파일에 심을 수 있는 손인가»가 갈린다** (2026-09-09에 웹을 짓다가 알았다).
     ⚠ **브라우저는 hwpx 파일을 못 고친다.** 그런데 코드는 **파일에 있어야** 뜻이 있다 —
       웹에서 코드를 매겨 창고에만 넣으면 파일은 여전히 코드가 없고, **내년에 그 문항이
       또 코드 없이 와서 또 새 코드를 받는다.** 중복을 막으려던 일이 중복을 만든다.
     🔵 그래서 **웹은 코드를 안 매기고 막는다**(`심을수있나:false`).
       코드 심기는 파일을 고치는 일이고, 그건 `tools/item-code.mjs` 의 몫이다. */
  ...
  if(!심을수있나){
    const 한벌 = v.코드잃음.length - 갈린것;
    if(한벌)             막.push({ 갈래:'코드 없음',   n:한벌,              말:'창고에 있는 문항인데 파일에 코드가 없습니다 — 코드를 심은 파일로 올려주세요' });
    if(v.새문항.length)  막.push({ 갈래:'코드 없는 새 문항', n:v.새문항.length, 말:'새 문항입니다 — 파일에 코드를 먼저 심어야 «내년에 다시 올려도» 이어집니다' });
  }
  return 막;
}
...
/* 🔴 **1990년대 기출도 온다** (2026-09-06 · 03.원의방정식에 `1994년 11월 11번` 이 있었다).
   `20[0-9]{2}` 만 받아서 그 세 문항의 출처를 «못 읽었고», 그러면 미주 135 대 출처 132 로
   수가 어긋나 **파일 전체가 멈췄다.** 한 해 표기 때문에 135제가 통째로 막힌 것이다.
   🔵 **코드 쪽은 이미 19xx 를 알고 있었다** — `srcCodeInfo` 가 `년<=30 ? 20xx : 19xx` 로 읽는다.
...
function hwpxParseSourceTag(s){
  ...
  /* 🔵 **형(가형·나형)도 읽는다** (2026-09-06 · 사용자가 정했다 — 「가=A · 나=B」).
     ...
     실측: 02단원 138·139·140번(정사각형 접기)과 03단원 243·244·245번(원)이 그랬다.
     ⚠ **교재에 안 적혀 있으면 못 읽는다** — 실측으로 02·03단원에 「가형/나형」이 0회였다.
       그래서 사용자가 «겹치는 것만» 출처에 적어 주기로 했다. 안 적힌 것은 형이 없는 것으로 둔다
       (그러면 지금까지 매긴 코드가 하나도 안 바뀐다).
     🔵 2014~2016 수능의 «A형/B형»도 같은 글자로 받는다 — 시행 시기가 달라 한 (년·월)에
...
function hwpxMakeSourceCodes(뽑은것){
  ...
  /* 🔴 **①꼴과 «같은» 변형 꼴을 쓴다** (2026-09-06 · 사용자가 정했다).
     처음에는 딱지를 코드에 그대로 넣었다 — `1050919OR` · `1050919NC01`.
     그랬더니 **변형인 `1050919NC01` 이 «원본»처럼 취급됐다**:
       · 창고가 「변형 없음」이라 말했다 — 교재에 둘이나 있는데
       · AI 가 그것을 못 보고 **이미 있는 변형을 또 만들었다**(실측 3건 · 하루 20건 한도를 태운다)
       · `storeSubjectCode`·`itemDocKey`·`srcSiblings` 세 자리가 ②꼴 때문에 조용히 갈렸다
     🔵 **정보는 하나도 안 잃는다** — NC→N · UP→U · DW→D 로 뜻이 그대로 옮겨간다.
       `1050919OR`   → `1050919`      (원본)
       `1050919NC01` → `1050919-N01`  (숫자변형)
       `1050919UP01` → `1050919-U01`  (상향)
       `2160321BNC01`→ `2160321B-N01` (나형의 숫자변형)
     ⚠ 번호는 갈래마다 따로 센다 — `splitItemCode` 가 «갈래+번호»로 읽기 때문이다. */
...
/* =================== 창고에 «이미 담긴» 글을 고친다 (2026-09-06 · 사용자 요청) ===================
   ...
   🔵 **왜 되는가** — 수식 규칙이 못 알아본 것은 «사라진» 것이 아니라 **날글자 그대로 남아**
     창고에 담겨 있다(`$RMO \left( 0,~0 \right)$`). 그러니 hwpx 를 다시 안 읽어도
     담긴 글에 그 규칙을 «한 번 더» 대면 그 자리에서 고쳐진다.

   🔴 **`convertHwpEq` 를 통째로 다시 대면 안 된다.** 그것은 «한글 수식 원문»을 받는 함수라
     이미 LaTeX 이 된 글에 대면 방금 만든 것을 또 바꾼다(이 파일이 곳곳에서 겪은 그 함정이다).
     → 그래서 **«못 알아봤던 것»만** 다시 댄다. 아래 셋이 그것이다.
   🔴 **`$…$` 안에서만 고친다.** 본문은 한글과 수식이 섞인 글이라 밖까지 손대면 «bar» 같은
     평범한 낱말을 물어뜯을 길이 생긴다. 수식은 언제나 `$` 안에 있다(실측).

   ⚠ **여기에 규칙을 «새로» 적지 말 것.** `convertHwpEq` 가 쓰는 것과 **같은** 규칙이어야 한다 —
     둘이 갈리면 새로 올린 교재와 고쳐 놓은 창고가 서로 다른 글이 된다.
   ⚠ 고칠 것이 없으면 **들어온 글을 그대로** 돌려준다 — 부르는 쪽이 «바뀌었나»로 가른다. */
