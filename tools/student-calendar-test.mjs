...
console.log('\n휴강 · 보강 — 추석에 수업이 있다고 말하면 안 된다\n');
{
  ...
  /* 🔵 **옮긴 날의 시간** (2026-09-20 · 사용자 — 「다른날짜로 옮기는 경우에 시간도 기록하고 싶어」).
     🔴 여태 보강은 «원래 요일»의 시간표를 그대로 들고 갔다 — 목요일 수업을 토요일로 옮기면
       실제로는 오전에 하는데 달력은 저녁이라고 말했다. 적었으면 그것이 이긴다. */
  const 시간옮김 = [{id:'o2t', date:'2026-09-21', label:'추석', classIds:[], moveTo:'2026-09-23', moveTime:'10:00~12:30'}];
  const g3 = 모은다(기본DATA(), 기본c(), '2026-09', {}, 시간옮김);
  봄('🔴 적어 둔 시간이 보강 줄에 선다', (g3['2026-09-23']||[])[0].sub.includes('10:00~12:30'), true);
  봄('🔴 원래 요일의 시간표는 «안» 쓴다 — 두 시간이 한 줄에 서면 어느 쪽인지 모른다',
     (g3['2026-09-23']||[])[0].sub.includes('19:00~21:00'), false);
  봄('   휴강 줄도 «몇 시로» 갔는지 말한다', (g3['2026-09-21']||[])[0].sub.includes('2026-09-23 10:00~12:30로'), true);
  봄('⚠ 안 적었으면 예전 그대로 — 원래 요일의 시간표다',
     (g2['2026-09-23']||[])[0].sub.includes('19:00~21:00'), true);
