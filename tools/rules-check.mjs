...
console.log('');
/* 🔵 **«전부 막힘»이 좋은 것인가 나쁜 것인가는 «언제냐»에 달렸다.**
   이 도구는 익명으로 묻는다. 2026-09-08에 제대로 된 규칙을 올린 뒤로는
   **전부 막히는 것이 옳다** — 사이트를 열기만 한 사람은 아무것도 못 봐야 한다.
   ⚠ 그전에는 맺음말이 「콘솔에서 열어야 한다」였다. 다 잘 된 날에 고치라고 재촉하는 꼴이었다.
     도구가 하는 말은 «잰 값»만큼 중요하다 — 값이 맞아도 말이 틀리면 사람이 잘못 움직인다. */
if (!막힌것.length) {
  console.log('  🔴 **전부 열려 있다 — 이건 나쁜 상태다.**');
  console.log('     익명이 다 읽고 쓸 수 있다는 뜻이다. 사이트를 열기만 한 사람도 그렇다.');
  console.log('     ▶ firestore.rules 를 콘솔에 붙여넣고 게시할 것.\n');
} else if (막힌것.length === 볼것.length) {
  console.log('  ✅ **익명에게 전부 막혀 있다 — 이게 옳은 상태다.**');
  console.log('     사이트를 열기만 한 사람은 아무것도 못 읽는다.');
  console.log('     🔵 로그인한 사람이 «제 몫만» 보는지는 이 도구로 못 잰다 — 그건 배포본에서 눈으로 볼 것.\n');
} else {
  console.log('  ⚠ **반만 막혀 있다** — 규칙이 덜 올라갔거나, 어떤 컬렉션이 규칙에서 빠졌다:');
  for (const x of 막힌것) console.log('     · ' + x.c + ' (' + x.이름 + ') — 읽기 ' + x.r + ' · 쓰기 ' + x.w);
  console.log('\n  ▶ 규칙에 없는 컬렉션은 기본이 «막힘»이다. firestore.rules 에 그 줄이 있는지 볼 것.\n');
}
