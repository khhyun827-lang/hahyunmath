...
console.log(NL + '⑤ 밑그림 — «확실한 것»만 깐다' + NL);
{
  ...
  const r = F.planSeed();
  봄('학생마다 «전날 직보» 한 칸씩 깔았다', r.깐것, 2);
  봄('🔴 시험 일정이 없는 학생은 못 깐다 — 그 수를 말한다', r.없음, 1);
  /* 🔴 **「수학시험」은 여기서 안 깐다** (2026-09-08 · 사용자가 「미리 표시」를 요청하면서 갈렸다).
     그 날은 시험 일정에서 나오는 값이라 표가 **바탕으로** 보여 준다. 찍어서 담으면
     같은 사실이 두 군데 살고, **시험 날짜를 고쳤을 때 표가 옛말을 한다.** */
  봄('🔴 수학 시험일은 «담지» 않는다 — 표가 바탕으로 미리 보여 준다',
     F.planCell('a','2026-06-25'), undefined);
