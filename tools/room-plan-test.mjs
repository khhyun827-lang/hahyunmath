...
console.log(NL + '④ 이번 주 게임 — 한 주를 콕 집는다' + NL);
{
  ...
  /* 🔴 **콕 집은 다음 주가 «또» 같은 게임이면 안 된다** (2026-09-21).
     09-14 를 집으면서 번갈이가 한 주 어긋나, 09-21 도 똥피하기가 됐다 — 두 주 연속이었다.
     ⇒ 기준(GAME_EPOCH)을 한 주 옮기고 지나간 주들을 GAME_PICKS 에 못박아 고쳤다.
     ⚠ 그러니 이 검사는 «이어지는 두 주가 다른 게임인가»를 묻는다. */
  const 이어짐 = ['2026-08-31', '2026-09-07', '2026-09-14', '2026-09-21', '2026-09-28', '2026-10-05']
    .map(m => G.gameOfWeek(m).name);
  봄('🔴 09-14 만 콕 집은 것이고 그 다음 주는 달리기로 돌아온다', 이어짐[3], '달리기');
  봄('🔴 같은 게임이 세 주 이어지지 않는다',
    이어짐.filter((n, i) => i >= 2 && n === 이어짐[i - 1] && n === 이어짐[i - 2]), []);
  봄('🔴 콕 집은 주(9/14) 말고는 꼬박꼬박 번갈아 든다',
    이어짐.filter((n, i) => i > 0 && n === 이어짐[i - 1]), ['똥피하기']);
  /* 🔴 지난 주들은 **있던 그대로**여야 한다 — 그 주 순위가 다른 게임 것으로 읽히면 안 된다.
     (기준을 옮겼으므로 지금은 GAME_PICKS 의 못이 이것을 지킨다) */
  봄('🔴 지난 주들은 그대로다 (뒤집히면 그 주 순위가 다른 게임 것이 된다)', G.gameOfWeek('2026-08-31').name, '달리기');
