...
const MODE_RUNNER = {
  ...
  /* 🔴 **점프 높이가 «화면 높이»에 매여 있었다** (2026-09-17 · 사용자 — 「마치 점프로 뛰어넘을 수
       있을 것처럼 판단돼 **점프력을 줄이던가 숙이는 장애물을 조금 높이던가** 해야할 것 같아」).
     예전에는 `1.10·h` 로 던지고 `3.54·h` 로 내렸다. 그런데 **사람 키는 «길 폭»에 매여 있다**
     (`92 · g.u` — S-D-11 이 세운 잣대). 잣대가 둘이라 화면 모양에 따라 사람이
     **제 키의 1.0배에서 1.6배까지** 제각각 뛰었다(길쭉한 화면일수록 높이 뛴다).
     그러니 어떤 화면에서는 무엇이든 뛰어넘을 수 있을 것처럼 보인다 — 짚어 주신 그 자리다.
   🔵 **꼭대기를 «제 키»로 적는다** — 어느 화면에서나 제 키의 1.25배까지 뛴다.
     ⚠ 떠 있는 시간(T)만은 **초**로 둔다 — 장애물이 다가오는 빠르기는 화면 크기와 상관없다.
       A = 1.25·92·u · T = .55초 → v = 4A/T · g = 2v/T (포물선에서 바로 나온다).
     ⚠ **물웅덩이 문턱도 같이 옮겼다**(`h·.055` → `48·u`) — 하나만 화면 잣대로 남으면
       화면에 따라 «뛰었는데 안 넘어지는» 판이 생긴다. 꼭대기의 .42배로 예전과 같은 비율이다. */
  T_AIR: .55,                                    // 떠 있는 시간(초)
  JUMP_H: 1.25 * 92,                             // 꼭대기 — «사람 키»의 배수 (바닥 기준 px)
  GOAL_U: 48,                                    // 물웅덩이를 넘었다고 보는 문턱 (바닥 기준 px)
  /* 점프 꼭대기 — «바닥 기준 px»과 화면 px 둘 다. **그림이 이 값을 보고 선다**(→ `gate`).
     🔵 잣대를 두 곳에 두지 않는다 — 점프를 손보는 날 그림이 거짓말을 하지 않게. */
  apexU(g){ return this.JUMP_H; },
  apexY(g){ return this.JUMP_H * (g.u || 1); },
  jumpV(g){ return 4 * this.apexY(g) / this.T_AIR; },
  jumpG(g){ return 2 * this.jumpV(g) / this.T_AIR; },
  up(g){ if(g.jy <= 0 && g.slide <= 0) g.jv = this.jumpV(g); },
  ...
  step(g, dt){
    ...
    if(g.spawnZ <= 0){
      ...
      lanes.forEach(l => {
        ...
        /* eng — «내 줄에서 맞이했다»는 표. 지나간 뒤(z<0)의 판정은 이 표가 있는 것만 본다 (09-17) */
        g.items.push({lane:l, z:this.Z_FAR, kind, hit:false, eng:false, seed: Math.random() * 6.2832});
      });
    ...
    for(let i = g.items.length - 1; i >= 0; i--){
      ...
      /* 🔴 **지나간 것에 «옆에서» 들어가 부딪히던 것** (2026-09-17 · 사용자가 짚었다 —
           「분명 그림은 지나갔는데 장애물에 걸린 판정이 나와」).
         띠(`|z| ≤ .075`)는 앞뒤로 같은데 **화면에서 «지나갔다»는 z<0 에서 이미 끝난다** —
         `proj` 가 z<0 을 사람보다 «아래»로 던지기 때문이다(z=-.075면 발밑 한참 아래에 크게).
         그래서 다른 줄에서 그것이 지나가는 것을 보고 그 줄로 옮기면, 눈에는 없는 것에 걸렸다.
       🔵 **뒤쪽 띠는 «그때 그 줄에서 맞이한 사람»만 본다.** 그것이 원래 뒤쪽 띠를 둔 까닭이다 —
         뛰어넘었다가 **그 위에 내려앉는** 것을 잡으려던 것이지, 지나간 뒤에 새로 들어온 사람을
         잡으려던 것이 아니다. 앞쪽(z ≥ 0)에서 이 줄에 있었으면 표를 찍고, 뒤쪽은 그 표만 본다.
       ⚠ 여기까지 왔다는 것은 **이미 줄이 겹친다**는 뜻이다(바로 위에서 걸렀다). */
      if(it.z >= 0) it.eng = true;                     // 앞에서 맞이했다 — 끝까지 본다
      else if(!it.eng) continue;                       // 이미 지나간 것 · 그때 나는 다른 줄이었다
      /* 🔵 **어떻게 피하는지가 종류마다 다르다** — 이것이 이 게임의 전부다 */
      const safe = it.kind === 'low'  ? (g.jy > this.GOAL_U * (g.u || 1))   // 낮은 것 — 뛰어넘는다
  ...
  gate(g, it, p, x, near){
    ...
    /* 🔴 **«아래로 지나는 것»으로 보여야 한다** (2026-09-17 · 사용자가 짚었다).
       예전에는 봉 하나(62~77)에 기둥 꼭대기가 82였다 — **사람 키(92)보다 낮았다.**
       점프 꼭대기는 150이었으니 뛰면 통째로 넘어가는 그림인데, 판정은 «미끄러져야 산다»였다.
       **그림이 거짓말을 하고 있었다** — 「점프로 넘을 수 있을 것처럼 보인다」는 그 말이다.
     🔵 봉을 «빗줄 판»으로 세운다 — 아래끝은 그대로 두고(숙여야 지난다) **꼭대기를
       «점프 꼭대기 위»로** 올린다. 뛰어도 사람이 판보다 위로 못 올라가는 것이 눈에 보인다.
     ⚠ 아래끝(HB)은 «숙인 머리»와 «선 머리» 사이에서만 움직인다 — 더 올리면 «서서도
       지나가지겠다»가 되고, 내리면 숙여도 머리가 뚫는다. 아래 🔴 항목이 그 두 값을 잰다.
     ⚠ 꼭대기는 **`apexU` 에서 뽑는다**(점프와 같은 잣대). 화면 모양에 따라 점프 높이가
       달라지므로 여기에 숫자를 적어 두면 어떤 화면에서는 또 낮아진다. */
    /* 🔴 **슬라이드하는 머리가 봉을 뚫고 있었다** (2026-09-17 · 사용자 — 「차단봉 밑으로 들어가는
       구멍 조금만 더 넓혀줘」). 스프라이트를 재 보니 **슬라이드 두 칸의 머리끝이 57.2 · 65.7px**
       이다(바닥 기준 · `tools/game-duck-gap-test.mjs` 가 그림에서 직접 잰다).
       아래끝이 62였으니 **둘째 칸에서는 머리가 봉 위로 나왔다** — 「구멍이 좁다」가 그 자리다.
     ⚠ 그래도 «서 있는 키»(87~90px)보다는 낮아야 한다 — 올리면 «서서도 지나가지겠다»가 된다.
       74면 숙인 머리 위로 한 뼘(8px) 뜨고, 선 머리보다는 아직 13px 아래다. */
    const HB = 74, dep = .018;                          // 판 아래끝 · 앞뒤 두께
    /* 꼭대기는 «뛴 발»보다 한 뼘 위다. 붙여 두면 «아슬아슬하게 넘겠는데»로 읽혀서,
       실제로 그렇게 나왔다(첫 판을 브라우저로 보고 30을 더 올렸다). 한 뼘 = 사람 키의 1/3. */
    const TOP = Math.max(HB + 46, Math.round(this.apexU(g)) + 30);          // 기둥 꼭대기
    const TH  = TOP - 5 - HB;                           // 판 두께 — 아래끝과 꼭대기 사이를 채운다
  ...
  player(g){
    ...
    /* ⚠ 여기도 «점프 꼭대기»를 잣대로 쓴다 (2026-09-17) — 예전에는 `h·.18` 이라고 적혀 있었는데
       그것이 옛 꼭대기(.171h)에 맞춘 숫자였다. 점프를 낮추면 그림자가 끝까지 안 줄어든다. */
    const up = Math.min(1, lift / this.apexY(g));
